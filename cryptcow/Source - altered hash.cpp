...
struct t_hash {
	...
	t_hash* next;
	t_hash() { val = 0; next = 0; }
} found[500000], conflicts[500000], * p_conflicts = conflicts;

const int maxlen = 80;
const int compset_size = 10000;
...
int ELFHash(const char* str, unsigned int length) {
	unsigned int hash = 0;
	unsigned int x;
...
bool solve(char* s, int slen, int a, int b, int level) {
	...
	int hI = ELFHash(s, slen) % 500000;
	int hV = SDBMHash(s, slen);
	t_hash* hP = found + hI;
	if (!(hP->val)) {
		hP->val = hV;
	}
